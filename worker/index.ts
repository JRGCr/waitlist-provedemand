...
async function handleSignup(request: Request, env: Env, corsHeaders: Record<string, string>): Promise<Response> {
  try {
    ...
    await env.DB.prepare(`
      INSERT INTO signups (email, name, company, referral_source, ip_address, user_agent)
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(
      body.email,
      body.name || null,
...
async function handleSignupsExport(request: Request, env: Env, corsHeaders: Record<string, string>): Promise<Response> {
  ...
  const dataQuery = `
    SELECT id, email, name, company, referral_source, created_at 
  ...
  if (format === 'csv') {
    const csv = [
      'id,email,name,company,referral_source,created_at',
      ...signups.results.map((s: any) => 
        `${s.id},"${s.email}","${s.name || ''}","${s.company || ''}","${s.referral_source || ''}","${s.created_at}"`
      )
    ].join('\n');
